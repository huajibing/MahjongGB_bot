...
def filterData():
    ...
    newoutcomes = [[] for i in range(4)]
    for i in range(4):
        for j, o in enumerate(obs[i]):
            if o['action_mask'].sum() > 1:
                newobs[i].append(o)
                newactions[i].append(actions[i][j])
                newoutcomes[i].append(outcomes[i][j])
    obs = newobs
    actions = newactions
    outcomes = newoutcomes

def saveData():
    assert [len(x) for x in obs] == [len(x) for x in actions] == [len(x) for x in outcomes], 'obs, actions, outcomes not matching!'
    ...
    np.savez('data/%d.npz'%matchid
        ...
        , outcome = np.array([x for i in range(4) for x in outcomes[i]])
    )
    for x in obs: x.clear()
    for x in actions: x.clear()
    for x in outcomes: x.clear()
...
with open('data/data.txt', encoding='UTF-8') as f:
    ...
    while line:
        ...
        if t[0] == 'Match':
            ...
            scores = [int(s) for s in t[1:]]
            final_outcomes = [0] * 4
            if not all(s == 0 for s in scores):
                winner = np.argmax(scores)
                loser = np.argmin(scores)
                final_outcomes[winner] = 1
                if scores[loser] < -8:
                    final_outcomes[loser] = -1
            
            for i in range(4):
                outcomes[i] = [final_outcomes[i]] * len(obs[i])
            filterData()
            saveData()
        line = f.readline()
